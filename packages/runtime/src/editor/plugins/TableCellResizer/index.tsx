 ...
 *
 */
import { $isTableNode, TableCellNode, TableDOMCell, TableMapType } from '@lexical/table';
...
function TableCellResizer({editor}: {editor: LexicalEditor}): JSX.Element {
            ...
            editor.registerNodeTransform(TableNode, (tableNode) => {
                ...
                if (tableElement && isHTMLElement(tableElement)) {
                    ...
                    if (isHTMLElement(table) && table.tagName === 'TABLE') {
                        // Apply auto layout when no widths are set
    ...
    const calculateColumnWidthsFromDOM = (
        ...
        if (!firstRow) {
            return Array(columnCount).fill(MIN_COLUMN_WIDTH);
        }
        ...
        for (let i = 0; i < columnCount; i++) {
            if (cells[i]) {
                const width = (cells[i] as HTMLElement).offsetWidth;
                widths.push(Math.max(width, MIN_COLUMN_WIDTH));
            } else {
        ...
        (widthChange: number) => {
            ...
            editor.update(
                () => {
                    ...
                    // If no widths are set yet, calculate them from the DOM
                    if (!colWidths) {
                        const tableElement = editor.getElementByKey(tableNode.getKey());
                        if (!tableElement) {
                            return;
                        ...
                        colWidths = calculateColumnWidthsFromDOM(tableElement, columnCount);

                        // Also update the table layout to fixed since we're now managing widths
                        const table = tableElement.querySelector('table') || tableElement;
                        if (isHTMLElement(table) && table.tagName === 'TABLE') {
                            table.style.tableLayout = 'fixed';
                        }
