...
export function iconForSharedDocument(
  doc: SharedDocument,
  descriptors: Parameters<typeof resolveSharedDocumentTypePresentation>[1] = [],
): string {
  return resolveSharedDocumentTypePresentation(doc, descriptors).icon;
}

function SharedDocIcon({ document, size, className }: {
  ...
  return (
    <MaterialSymbol
      icon={resolveSharedDocumentTypePresentation(
        document,
...
const DocRow: React.FC<{
...
}> = ({ doc, favorited, onOpen, freshness, onToggleFavorite }) => (
  <button
    type="button"
  ...
  >
    <span className="shrink-0 w-7 h-7 rounded-md bg-[var(--nim-bg-tertiary)] flex items-center justify-center text-[var(--nim-primary)]">
      <SharedDocIcon document={doc} size={17} />
    </span>
    ...
    <StarToggle documentId={doc.documentId} favorited={favorited} onToggle={onToggleFavorite} />
...
const ChangedCard: React.FC<{
  ...
  onOpen: (doc: SharedDocument) => void;
  onToggleFavorite: (documentId: string) => void;
}> = ({ doc, freshness, favorited, onOpen, onToggleFavorite }) => (
  <button
    type="button"
  ...
  >
    <div className="flex items-start justify-between mb-2.5">
      <span className="w-[34px] h-[34px] rounded-md bg-[var(--nim-bg-tertiary)] flex items-center justify-center text-[var(--nim-primary)]">
        <SharedDocIcon document={doc} size={19} />
      </span>
...
export const SharedDocsHome: React.FC<SharedDocsHomeProps> = () => {
  ...
  const trashedDocs = useAtomValue(session.atoms.trashedSharedDocuments);
  const teamSyncStatus = useAtomValue(session.atoms.syncStatus);
  const favoriteDocs = useAtomValue(session.atoms.favoriteDocuments);
  const recentDocs = useAtomValue(session.atoms.recentDocuments);
  const changedDocs = useAtomValue(session.atoms.changedDocuments);
  ...
  const handleToggleFavorite = useCallback((documentId: string) => {
    session.toggleFavorite(documentId);
  }, [session]);
  ...
  const sortedAllDocs = useMemo(() => {
    ...
    if (allSort === 'name') {
      ...
      copy.sort(
        (a, b) =>
          resolveSharedDocumentTypePresentation(a, descriptors).typeLabel.localeCompare(
            resolveSharedDocumentTypePresentation(b, descriptors).typeLabel,
          ) ||
  ...
  }, [allDocs, allSort, descriptors, documentTypesRevision]);
  ...
  const handleCleanup = useCallback(async () => {
    if (teamSyncStatus !== 'connected' || cleanupProgress || !controller.cleanupEmptyDocuments) return;
    const result = await controller.cleanupEmptyDocuments(scope, allDocs, setCleanupProgress);
    setCleanupProgress(null);
    if (result.failed > 0) {
      host.notify?.({ level: 'warning', title: 'Cleanup finished with skipped documents', message: `Moved ${result.moved} empty document${result.moved === 1 ? '' : 's'} to Trash; ${result.failed} could not be verified.`, duration: 7000 });
    } else {
      host.notify?.({ level: 'info', title: 'Empty-document cleanup finished', message: `Moved ${result.moved} empty document${result.moved === 1 ? '' : 's'} to Trash.`, duration: 5000 });
    }
  }, [allDocs, cleanupProgress, controller, host, scope, teamSyncStatus]);

  const openDocument = useCallback((document: SharedDocument) => {
    host.openArtifact({
      kind: 'document',
      scope,
            ...
            <div className="rounded-lg border border-[var(--nim-border)] bg-[var(--nim-bg-secondary)] divide-y divide-[var(--nim-border)]">
              {trashedDocs.map((doc) => (
                <div key={doc.documentId} className="shared-docs-trash-row flex items-center gap-3 px-3 py-2.5">
                  <SharedDocIcon document={doc} size={18} className="text-[var(--nim-text-muted)]" />
                  ...
                  <button
                    ...
                    onClick={() => session.restoreDocument(doc.documentId)}
                  >
                    Restore
                  </button>
                  <button
                    type="button"
                    className="p-1 rounded border-none bg-transparent text-[var(--nim-error)] cursor-pointer hover:bg-[var(--nim-bg-hover)] disabled:opacity-50"
                    disabled={teamSyncStatus !== 'connected'}
      ...
      <div className="max-w-[860px] mx-auto">
        <div className="flex items-start justify-between gap-4 mb-5">
          ...
          <div className="flex items-center gap-2">
            {controller.cleanupEmptyDocuments ? (
              <button
                type="button"
