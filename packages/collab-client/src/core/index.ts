...
export type CollabCapabilityAvailability<TCapability> =
  | { status: 'available'; capability: TCapability }
  | { status: 'unavailable' };
...
export class CollabScopeResolutionError extends Error {
  ...
  constructor(message: string, options: { retryable: boolean; cause?: unknown }) {
    super(message);
    this.name = 'CollabScopeResolutionError';
...
export type CollabDataChange<TItem, TContainer> =
  | { type: 'snapshot'; snapshot: CollabDataSnapshot<TItem, TContainer> }
  | { type: 'items-upserted'; items: TItem[] }
  | { type: 'items-removed'; itemIds: string[] }
  | { type: 'containers-upserted'; containers: TContainer[] }
  | { type: 'containers-removed'; containerIds: string[]; itemIds: string[] }
  | { type: 'status'; status: CollabConnectionStatus };
 ...
 */
export interface CollabDataSource<
  TItem,
  TContainer,
  TCommand extends CollabCommand = CollabCommand,
  TResult extends CollabCommandResult = CollabCommandResult,
> {
...
export interface CollabDocsReadReceiptCapability {
  snapshot(scope: CollabScope): Promise<Array<ReadReceipt & { entityId: string }>>;
  subscribe?(
    scope: CollabScope,
    cb: (row: ReadReceipt & { entityId: string }) => void,
  ): Unsubscribe;
  markViewed(input: {
    scope: CollabScope;
    documentId: string;
...
export interface CollabDocsCapability<
  TItem = unknown,
  TContainer = unknown,
  TCommand extends CollabCommand = CollabCommand,
  TResult extends CollabCommandResult = CollabCommandResult,
...
export interface CollabHost<
  TDocuments extends CollabDocsCapability = CollabDocsCapability,
> {
