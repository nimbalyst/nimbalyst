...
describe("embedded document cascade sharing", () => {
  ...
  it("ignores block links inside fenced code blocks", async () => {
    ...
    const rewritten = rewriteEmbeddedDocumentLinks({
      ...
      sharedReferences: new Map([
        [
          "/workspace/docs/wireframe.mockup.html",
          { documentId: "mockup-1", orgId: "team-1" },
        ],
        [
          "/workspace/docs/example.mockup.html",
          { documentId: "mockup-2", orgId: "team-1" },
        ],
      ]),
    });
    ...
    expect(rewritten).toContain(
      "[Real embed](nimbalyst://doc/mockup-1?orgId=team-1"
    );
  });
  ...
  it("rewrites selected embeds with canonical deep links while preserving attrs", () => {
    ...
    const rewritten = rewriteEmbeddedDocumentLinks({
      ...
      sharedReferences: new Map([
        ...
        [
          "/workspace/sheets/forecast.calc.md",
          { documentId: "sheet/1", orgId: "team-1" },
        ],
      ]),
    });
  ...
  it("cascades canvas file cards while preserving their local reference", async () => {
      ...
      nodes: [
        {
          ...
          width: 300,
          height: 200,
          file: "design/screen.mockup.html",
          "x-nimbalyst": {
            reference: { kind: "file", path: "design/screen.mockup.html" },
          },
        },
        {
          id: "screen-b",
    ...
    const candidates = await discoverCanvasEmbeddedDocuments({
      canvas,
      sourceFilePath: "/workspace/board.canvas",
      workspacePath: "/workspace",
      catalog: catalog() as never,
      expectedOrgId: "team-1",
      fileExists: async () => true,
    ...
    expect(candidates[0]).toMatchObject({
      absolutePath: "/workspace/design/screen.mockup.html",
      occurrences: 2,
    });
    ...
    const rewritten = JSON.parse(
      rewriteCanvasEmbeddedDocuments({
        ...
        sharedReferences: new Map([
          [
            "/workspace/design/screen.mockup.html",
            { documentId: "screen/doc", orgId: "team-1" },
          ],
        ]),
      })
    );
    for (const rewrittenNode of rewritten.nodes) {
      expect(rewrittenNode.file).toBe("design/screen.mockup.html");
      expect(rewrittenNode["x-nimbalyst"].reference).toEqual({
        kind: "file",
        path: "design/screen.mockup.html",
        sharedAs: {
          uri: "nimbalyst://doc/team-1/screen%2Fdoc",
        },
      });
    }
  });
  ...
  it("shares each selected local document once without opening child tabs and tolerates partial failure", async () => {
    ...
    const result = await shareEmbeddedDocuments({
      ...
      readSourceContent: vi.fn(
        async (candidate) => `content:${candidate.fileName}`
      ),
      createDocument,
      generateId: vi
        .fn()
    ...
    expect(
      result.sharedReferences.get("/workspace/sheets/forecast.calc.md")
    ).toEqual({
      documentId: "sheet-1",
      orgId: "team-1",
    });
  ...
  it("does not report a reused already-shared document as newly created", async () => {
    ...
    expect(
      result.sharedReferences.get("/workspace/docs/wireframe.mockup.html")
    ).toEqual({
      documentId: "existing-doc",
      orgId: "team-1",
    });
