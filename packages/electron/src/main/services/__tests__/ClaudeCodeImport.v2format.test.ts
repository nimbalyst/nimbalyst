...
const PERSISTED_FULL_OUTPUT =
  "FULL EXTERNAL TOOL OUTPUT, much larger than 2KB in real life.";
...
describe("Claude Code v2 sync", () => {
  ...
  it("imports follow-up user prompts as input direction even when parentUuid is set", async () => {
    ...
    const meta = await extractSessionMetadata(filePath);
    expect(meta).not.toBeNull();
    await syncSessions(realSessions, {} as any, [meta!]);

    const stored = (
      await db.query<{ direction: string; content: string }>(
        "SELECT direction,content FROM ai_agent_messages ORDER BY id"
      )
    ).rows;
    const inputs = stored
      ...
      .map((m) => {
        try {
          return JSON.parse(m.content).prompt as string;
        } catch {
          return "";
        }
      });
  ...
  it("skips CLI bookkeeping (slash commands, command output) wrapped in user-role messages", async () => {
    ...
    const inputs = stored
      ...
      .map((m) => {
        try {
          return JSON.parse(m.content).prompt;
        } catch {
          return "";
        }
      });
  ...
  it("inlines persisted-output references and ingests subagent jsonl as parent-session messages", async () => {
    ...
    expect(meta).not.toBeNull();
    const results = await syncSessions(realSessions, {} as any, [meta!]);
    expect(results[0].success).toBe(true);

    // Session row should preserve the Claude Code session id verbatim so the
    // SDK can resume it.
    const createdSession = (await realSessions.get(
      (
        await db.query<{ id: string }>("SELECT id FROM ai_sessions")
      ).rows[0].id
    ))!;
    const stored = (
      await db.query<{ direction: string; content: string }>(
    ...
    const inlined = stored.find((m) =>
      m.content.includes(PERSISTED_FULL_OUTPUT)
    );
    expect(
      inlined,
      "expected persisted-output content to be inlined"
    ).toBeDefined();
    ...
    expect(
      subagentMessage,
      "expected at least one subagent-tagged assistant message"
    ).toBeDefined();
  });
});
