...
export interface QueuedPromptsStore {
  ...
  /** Terminally fail pending rows when their session cannot be routed to a workspace. */
      ...
      const { rows } = await db.query<any>(
        `UPDATE queued_prompts
         SET interrupt_receipt = $3,
             ...
             completed_at = CASE WHEN $4 THEN completed_at ELSE CURRENT_TIMESTAMP END,
             error_message = CASE WHEN $4 THEN error_message ELSE COALESCE($5, 'priority interrupt rejected') END
         WHERE id = $1
