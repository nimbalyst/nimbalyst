...
export function createPGLiteQueuedPromptsStore(
  ...
  return {
    async create(input: CreateQueuedPromptInput): Promise<QueuedPrompt> {
      ...
      // This UPSERT is the sole allocator for a source session. Deriving a
      // floor from durable rows makes upgrades safe even when an older schema
      // backfilled sequences before the allocator table existed. It remains
      ...
      const allocation = await db.query<{ submission_sequence: number }>(
        `INSERT INTO queued_prompt_source_sequences (source_session_id, next_sequence)
         SELECT $1, COALESCE(MAX(submission_sequence), 0) + 2
         FROM queued_prompts
         WHERE source_session_id = $1
         ON CONFLICT (source_session_id)
         DO UPDATE SET next_sequence = GREATEST(
           queued_prompt_source_sequences.next_sequence + 1,
           (
             SELECT COALESCE(MAX(submission_sequence), 0) + 2
             FROM queued_prompts
             WHERE source_session_id = $1
           )
         )
      ...
      if (rows.length === 0) {
        ...
        if (
          row.clientSubmissionId !== truth.clientSubmissionId ||
          row.sessionId !== input.sessionId ||
          row.sourceSessionId !== truth.sourceSessionId ||
          row.sourceRoomId !== truth.sourceRoomId ||
          row.producer !== truth.producer
        ) {
          throw new Error('Queued prompt submission identity conflict');
        }
        if (!row.payloadReceipt || !payloadReceiptsMatch(input.prompt, row.payloadReceipt)) {
