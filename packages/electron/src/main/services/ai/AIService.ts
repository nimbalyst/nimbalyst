...
export class AIService {
  ...
  // sessionsProcessingQueue -- that set's ownership belongs to the
  // queued-prompt dispatch chain, and a direct send is neither a dispatch nor
  // an interrupt. Checked alongside sessionsProcessingQueue.has() so a direct
  // send still cannot overlap a queue-driven turn for the same session.
   ...
   * as the queued-prompt dispatcher so this calls the identical
   * sendMessageHandler (= streamingHandler.handle) that both normal chat
   * input and queued prompts already use.
   */
  public async sendMessageDirect(
    ...
    // whether owned by the queued-prompt dispatch chain
    // (sessionsProcessingQueue) or another concurrent direct send
    // (directSendInFlight) -- see that field's own comment for why these are
    // separate guards rather than one shared set.
    if (this.sessionsProcessingQueue.has(sessionId) || this.directSendInFlight.has(sessionId)) {
