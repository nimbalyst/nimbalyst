...
export async function flushNextClaudeCliQueuedPrompt(
  ...
  // Final admission is the store's single atomic recovery-gated claim. A
  // marker installed after listPending therefore leaves the row pending.
  ...
  try {
    ...
    // Renderer notification is best-effort and never owns queue settlement.
  }
  ...
  try {
    ...
    if (!prompt.trim() && (!attachments || attachments.length === 0)) {
      const rejected = await deps.failAfterDispatch(
        claimed.id,
        'Queued CLI prompt had no sendable content',
        args.sessionId,
        claimToken,
      );
    ...
    if (!submitted) {
      const rejected = await deps.failAfterDispatch(
        claimed.id,
        'CLI prompt submission produced no terminal input',
        args.sessionId,
        claimToken,
      );
      if (!settlementAccepted(rejected)) {
    ...
    const completion = await deps.completeAfterDispatch(claimed.id, args.sessionId, claimToken);
    if (!settlementAccepted(completion)) {
      throw new QueueSettlementRejectedError(`Queued prompt completion was rejected: ${completion.outcome}`);
    }
    return true;
  } catch (error) {
