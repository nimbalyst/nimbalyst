...
### Visible-session context and Live prompt answers

Navigation publishes the selected computer, project, session title/identity, and resolution state as one observation. A session becomes a default target only after its cached ownership matches the selected computer and project. A list or document screen clears that target. Navigation updates are silent and reach both Live speech context and its managed controller; `get_current_context` recovers the latest snapshot. A previously queued task and a question already being read retain their original source even when the user opens another session. Desktop summaries identify their source; offline cached summaries explicitly say that pending approval state is unknown.

Live's `read_pending_prompt` negotiates `promptAnswersVersion: 1` with the source desktop. The host returns one canonical pending prompt and a content-versioned presentation lease. The app reads the complete bounded question using `AVSpeechSynthesizer`, including a commit's message and file list or a permission's command and warnings. The [Live API reference](https://developers.openai.com/api/reference/python/resources/live) exposes incremental output transcripts without a complete-turn boundary; Live audio buffer drains and announcement dismissal therefore do not grant answer authority. Native utterance completion does. While it reads, the microphone is gated; tap the microphone to interrupt. A route change, interruption, background transition, connection replacement, failed renewal, or changed proposal invalidates presentation.

After verified playback, `answer_prompt` uses fresh microphone transcript fragments captured by the app. The model cannot supply the source prompt, presentation token, or answer text. Explicit whole-utterance approval/cancellation and exact option labels are supported; qualified or ambiguous approvals fail without dispatch. Simple free-text questions are supported, but multi-select/multi-question forms, unsupported permission schemas, and proposals exceeding the bounded readout use their existing app card. Spoken permissions are one-time only. Asking to prepare a commit submits a coding task; it is not approval.

The host revalidates the exact prompt and ownership before delivery. Cards and voice share response reservations; commits share a reservation before Git execution. A successful answer receipt is distinct from task completion, and a commit receipt contains the actual Git result. `get_prompt_answer_status` queries the retained source receipt after a lost reply without replaying the mutation. Unknown outcomes require checking the card/Git history; a new proposal is required after an already-consumed commit proposal fails. Existing Realtime behavior remains available, and older desktops expose a card fallback instead of an unscoped mutation.

This remains part of the opt-in Live preview. Unit and simulator checks exercise context changes, source separation, interrupted playback state, canonical prompt validation, and duplicate delivery. Physical microphone/speaker/headset behavior and a paid Live conversation through the personal sync relay still require acceptance testing; those checks are not established by simulator results.
