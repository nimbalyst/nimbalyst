...
| `ai_mode_changed` | `SessionTranscript.tsx` | User switches session mode via ModeTag or Shift+Tab (Claude Code only) | `from` (planning/agent/auto)<br/>`to` (planning/agent/auto)<br/>`provider` (string)<br/>`session_id` (string) | (pending release) |  |
