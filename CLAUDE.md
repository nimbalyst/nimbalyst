...
| [SESSION_HIERARCHY.md](./docs/SESSION_HIERARCHY.md) | Authoritative rules for `ai_sessions` nesting: the two-layer invariant (root → children only, never grandchildren), the "worktree IS the workstream" rule, the valid combinations of `session_type` / `parent_session_id` / `worktree_id`, and the code paths that enforce them. | Creating any new session, parenting a session under another, adding a session-grouping derivation, or debugging "where did my sessions go in the left pane?" bugs. |
