...
- Editor tab git indicator now clears for files in nested git repos inside a non-git workspace root. `GitStatusService.getFileStatus` was bailing out whenever the workspace root had no `.git`, so files in nested repos were always reported as untracked even after being committed. The service now walks up from each requested file (bounded by the workspace root) to find that file's owning git root, groups requested files by owning root, and runs `git status --porcelain` once per root with the right `cwd`. Files outside any git repo within the workspace continue to surface as untracked. Adds `findGitRootForFile` helper plus 7 unit tests covering nested-repo, prefix-collision, and out-of-boundary edge cases. Fixes #122. Note: `getUncommittedFiles` and `getAllFileStatuses` (workspace-wide queries) still return empty for non-git workspace roots; nested-repo support there needs a directory-level repo discovery pass and is intentionally out of scope here.
