...
- Background-poll auto-updates now download silently in line with Greg's #314 sign-off ("plan to move toward auto-download in the future"). `autoUpdater.autoDownload` flips from `false` to `true`, and the `update-toast:show-available` toast is gated on the manual-check path via a new `shouldShowAvailableToast(wasManualCheck)` helper in `autoUpdaterUtils.ts`. Auto-poll updates now surface only the post-download "Ready to install" toast; the manual "Check for Updates" menu item still confirms the find before the renderer transitions through 'downloading' on the first progress event. Pure-function helper so the gate stays unit-testable without booting an Electron app global (3 cases in `autoUpdater.shouldShowAvailableToast.test.ts`). Follow-up to #245 / #314.
<!-- Bug fixes go here -->
