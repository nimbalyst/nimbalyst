...
Completed in the first fix pass:

- Docs examples and templates now pass the required `entry` to `createExtensionConfig`.
- Public editor examples/docs now use `EditorHostProps` instead of the removed `CustomEditorProps` flow.
- Public AI tool examples/docs now use `AIToolContext`, filesystem services, and the `ExtensionToolResult` shape.
- Example package versions and dev-kit templates were updated to the SDK's current Vite baseline.
- The SDK now owns the shared extension/editor type surface, and runtime re-exports those common types instead of maintaining a parallel fork.
- Manifest docs, runtime validation, and dev-server validation now agree on the current slash-command names, `fileIcons` shape, and optional `apiVersion`.
- `api-reference.md` was rewritten against the current SDK surface instead of the older hand-written subset.
- The SDK package now has a package-level README plus repository/homepage/bugs metadata for npm publishing.
- CI now includes a repeatable public-surface smoke check for the SDK package, docs examples, and extension-dev-kit.

Current implementation rule:

- `packages/extension-sdk/src/types/*` is the canonical public contract.
- `packages/runtime/src/extensions/*` should only add runtime-only loaded-instance types and host behavior on top of that shared contract.
...
- [x] Reconcile `packages/extension-sdk` types with `packages/runtime/src/extensions/*`.
- [x] Decide the canonical AI tool contract and remove or hard-deprecate conflicting names.
- [x] Decide the canonical manifest schema and make runtime validation, dev validation, docs, and typings match it.
- [x] Decide whether `apiVersion` is required; enforce that consistently everywhere.
...
- [x] Rewrite `getting-started.md` to use `EditorHostProps`.
- [x] Rewrite `custom-editors.md` to use the push/save-request model.
- [x] Rewrite `ai-tools.md` to use `AIToolContext` and the real result shape.
- [x] Rewrite `api-reference.md` from actual exported types, not hand-written snippets.
- [x] Rewrite `manifest-reference.md` from the real manifest schema.
- [x] Update all three examples so they build and typecheck.
...
- [x] Update `extension-dev-kit` templates to the current editor and tool APIs.
- [x] Update the `/new-extension` command guidance to match the final public contract.
- [x] Make the dev-kit scaffold the same package versions you expect public users to install.
- [ ] Add template smoke tests that scaffold, build, and typecheck each template.

### Packaging and publishing

- [x] Add a package-level README to `packages/extension-sdk`.
- [ ] Verify npm metadata:
- [ ] package name
- [ ] description
- [ ] repository
...
- [ ] Add CI jobs for:
- [x] `packages/extension-sdk` build/typecheck
- [x] example build/typecheck
...
- [x] Verify at least one binary/custom editor and one AI-tool-only extension in the shipping app.
